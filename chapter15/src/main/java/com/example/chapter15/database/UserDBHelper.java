...
public class UserDBHelper extends SQLiteOpenHelper {
    ...
    private static final String DB_PASSWORD = "android"; //数据库密码
    ...
    private UserDBHelper(Context context, int version) {
        ...
        SQLiteDatabase.loadLibs(context); // 使用sqlcipher必须事先加载依赖库
    }
    ...
    public SQLiteDatabase openReadLink() {
        if (mDB == null || !mDB.isOpen()) {
            // sqlcipher需要密码才能打开数据库
            mDB = mHelper.getReadableDatabase(DB_PASSWORD);
        }
        return mDB;
    }
    ...
    public SQLiteDatabase openWriteLink() {
        if (mDB == null || !mDB.isOpen()) {
            // sqlcipher需要密码才能打开数据库
            mDB = mHelper.getWritableDatabase(DB_PASSWORD);
        }
        return mDB;
    }
